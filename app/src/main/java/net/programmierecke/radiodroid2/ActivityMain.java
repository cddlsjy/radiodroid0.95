 ...
 */
                            ...
                            try {
                                ...
                                if (outputStream != null) {
                                    ...
                                    if (selectedMenuItem == R.id.nav_item_starred) {
                                        ...
                                        CustomStationManager customStationManager = radioDroidApp.getCustomStationManager();
                    ...
                    protected Void doInBackground(Void... params) {
                        try {
                            ...
                            if (currentMenuItem == R.id.nav_item_custom) {
                                CustomStationManager customStationManager = radioDroidApp.getCustomStationManager();
                                List<DataRadioStation> stations = customStationManager.LoadM3UReader(reader);
                                if (stations != null) {
                                    importedCount = stations.size();
                                    customStationManager.addMultiple(stations);
                                } else {
                                ...
                                HistoryManager historyManager = radioDroidApp.getHistoryManager();
                                List<DataRadioStation> stations = historyManager.LoadM3UReader(reader);
                                if (stations != null) {
                                    importedCount = stations.size();
                                    historyManager.addMultiple(stations);
                                } else {
                                    exception = new Exception(getResources().getString(R.string.error_import_failed_parse));
                                }
                            } else {
                                ...
                                if (stations != null) {
                                    importedCount = stations.size();
                                    favouriteManager.addMultiple(stations);
                                } else {
                                    exception = new Exception(getResources().getString(R.string.error_import_failed_parse));
                                }
                            }
    ...
    void LoadFavourites() {
        ...
        // 使用系统文件选择器，支持多种M3U相关MIME类型
        Intent intent = new Intent(Intent.ACTION_OPEN_DOCUMENT);
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        intent.setType("*/*");
        intent.putExtra(Intent.EXTRA_MIME_TYPES, new String[]{"audio/x-mpegurl", "application/x-mpegurl", "text/plain", "audio/mpegurl"});
    ...
    private void playRandomStation() {
        if (Utils.isOfflineMode(this)) {
            Toast.makeText(this, R.string.error_offline_mode_random_play, Toast.LENGTH_SHORT).show();
            return;
        }
