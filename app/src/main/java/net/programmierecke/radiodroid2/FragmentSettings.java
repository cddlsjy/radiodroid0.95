...
public class FragmentSettings extends PreferenceFragmentCompat implements SharedPreferences.OnSharedPreferenceChangeListener, IApplicationSelected, PreferenceFragmentCompat.OnPreferenceStartScreenCallback  {
    ...
    public void onCreatePreferences(Bundle bundle, String s) {
        ...
        if (s == null) {
            ...
            findPreference("export_database").setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
                @Override
                public boolean onPreferenceClick(Preference preference) {
                    // 使用 Storage Access Framework (SAF) 打开文件保存对话框（Android 10+ 推荐方式）
                    Intent intent = new Intent(Intent.ACTION_CREATE_DOCUMENT);
                    intent.addCategory(Intent.CATEGORY_OPENABLE);
                    intent.setType("application/octet-stream");
                    intent.putExtra(Intent.EXTRA_TITLE, "radio_droid_database.db");
            ...
            findPreference("import_database").setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
                ...
                public boolean onPreferenceClick(Preference preference) {
                    ...
                    confirmBuilder.setPositiveButton(getString(R.string.import_database_button), (dialog, which) -> {
                        // 使用 SAF 打开文件选择器（Android 10+ 推荐方式）
    ...
    */
    ...
    // 使用 SAF 导出数据库到指定 URI（Android 10+ 推荐方式）
    private void exportDatabaseToUri(Uri destinationUri) {
        // 显示进度对话框
        ...
        new Thread(() -> {
            try {
                ...
                // 通过 ContentResolver 写入到目标 URI
                try (OutputStream outputStream = requireContext().getContentResolver().openOutputStream(destinationUri);
                     FileInputStream inputStream = new FileInputStream(mainDatabaseFile)) {
                ...
                requireActivity().runOnUiThread(() -> {
                    ...
                    successBuilder.setMessage(getString(R.string.export_success_message, destinationUri.toString()));
    ...
    // 导出主数据库到外部存储（旧版方法，保留用于兼容性）
    private void exportDatabase() {
        // 旧版方法不再使用，显示错误提示
        Toast.makeText(requireContext(), R.string.error_export_method_deprecated, Toast.LENGTH_SHORT).show();
    }
